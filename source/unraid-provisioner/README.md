...
- Both forms have an **"Anonymize secrets in output"** checkbox. When checked, the saved profile is redacted in two ways: environment variables whose key name looks like a secret (password, token, API key, etc.) get a `CHANGE_ME` placeholder, and environment *values* are scanned for well-known personal data regardless of the key they sit under -- credentials embedded in connection strings (`scheme://user:pass@host`), private (RFC1918) IPv4 addresses (mapped consistently to fake `192.168.*` addresses), email addresses (mapped consistently to `userN@example.com`), and personal DDNS hostnames (rewritten to `*.example.duckdns.org`). Loopback/bind addresses (`127.0.0.1`, `0.0.0.0`) are left untouched. Useful before committing a profile to a shared/public Git repository. This is a best-effort keyword/pattern match, not a guarantee; review the output before sharing it.
